...
STRUCTURE_REFERENCE_PATH = (
    BASE /
    "view_behind_structure_reference_v2_current_pose/rgb.jpg"
)
...
structure_reference = cv2.imread(
...
if structure_reference is None:
    raise SystemExit(
        f"Missing current BEHIND structure reference: "
        f"{STRUCTURE_REFERENCE_PATH}"
    )

if structure_reference.shape[:2] != (720, 1280):
    raise SystemExit(
        f"Unexpected structure reference size: "
        f"{structure_reference.shape}"
    )
...
latest_status = {
    "schema":
        "harmony_pre_roof_view_behind_runtime_v5",
