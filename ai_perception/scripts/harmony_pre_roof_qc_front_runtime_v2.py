...
STRUCTURE_REFERENCE_PATH = (
    BASE /
    "view_front_structure_reference_v2_current_pose/rgb.jpg"
)

STRUCTURE_THRESHOLD_PATH = (
    BASE /
    "view_front_structure_thresholds_v2_current_pose.json"
)
...
if structure_ref is None:
    raise SystemExit(
        f"ABORT: missing FRONT structure reference: "
        f"{STRUCTURE_REFERENCE_PATH}"
    )

if structure_ref.shape[:2] != (720, 1280):
    raise SystemExit(
        f"ABORT: bad FRONT structure reference shape: "
        f"{structure_ref.shape}"
    )

# Reference was captured directly from /raw.
# FRONT runtime canonical view is ROTATE_180.
...
structure_cfg = json.loads(
    STRUCTURE_THRESHOLD_PATH.read_text(
        encoding="utf-8"
    )
)
...
latest_status = {
    "schema":
        "harmony_pre_roof_view_front_runtime_v2",
...
def structure_match(
    ...
    cur = structure_feature(
        current
    )

    ref = structure_feature(
        reference
    )
    ...
    if (
        ...
        raise RuntimeError(
            f"FRONT structure search too small: "
            f"{roi['name']}"
        )
...
def evaluate_roi_v2(
    ...
    # ---------------------------------------------
    # Legacy V1 values remain diagnostic only.
    # ---------------------------------------------
    ...
    # ---------------------------------------------
    # V2 final decision:
    # brightness-insensitive structure correlation.
    # ---------------------------------------------
    ...
    return {
        ...
        "rgb": {
            "structure_best_match": {
                ...
                "mode":
                    "GRADIENT_NORMALIZED_CORRELATION",
            },
    ...
    while True:
        ...
        try:
            ...
            for r in ROIS:
                ...
                roi_results[name] = evaluate_roi_v2(
                    name,
                    rgb,
                    depth,
                    r,
            ...
            status = {
                ...
                "decision_policy":
                    "GRADIENT_STRUCTURE_CORRELATION_V2",
