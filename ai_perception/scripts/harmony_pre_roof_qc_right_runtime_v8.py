...
STRUCTURE_REFERENCE_PATH = (
    ROOT /
    "datasets/harmony_parts_v1/pre_roof_qc/"
    "view_right_structure_reference_v3_current_pose/rgb.jpg"
)

STRUCTURE_THRESH_PATH = (
    ROOT /
    "datasets/harmony_parts_v1/pre_roof_qc/"
    "view_right_structure_thresholds_v4_current_pose.json"
)
...
structure_ref = cv2.imread(
    str(STRUCTURE_REFERENCE_PATH),
    cv2.IMREAD_COLOR
)

if structure_ref is None:
    raise SystemExit(
        f"ABORT: missing RIGHT structure reference: "
        f"{STRUCTURE_REFERENCE_PATH}"
    )

if structure_ref.shape[:2] != (720, 1280):
    raise SystemExit(
        f"ABORT: bad RIGHT structure reference shape: "
        f"{structure_ref.shape}"
    )

# Reference was captured directly from Robot Control /raw.
# RIGHT runtime canonical image is ROTATE_180.
structure_ref = cv2.rotate(
    structure_ref,
    cv2.ROTATE_180
)

structure_cfg = json.loads(
    STRUCTURE_THRESH_PATH.read_text(
        encoding="utf-8"
    )
)
...
latest_status = {
    "schema": "harmony_pre_roof_view_right_runtime_v8",
    "inspection": "PRE_ROOF",
    "view": "RIGHT",
...
def evaluate(rgb, depth):
    ...
    # ==================================================
    # RIGHT V8 WALL_BODY STRUCTURAL OVERRIDE
    #
    # Legacy RGB / Depth image differences remain visible
    # as diagnostics, but do not decide WALL_BODY anymore.
    # ==================================================

    wall_structure = structure_match(
        rgb,
        structure_ref,
        roi_by_name["WALL_BODY"]
    )

    wall_min_score = float(
        structure_cfg[
            "wall_body"
        ][
            "min_score"
...
def worker():
    ...
    while running:
        ...
        try:
            ...
            status = {
                "schema":
                    "harmony_pre_roof_view_right_runtime_v8",
