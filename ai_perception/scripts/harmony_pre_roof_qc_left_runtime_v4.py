...
STRUCTURE_REFERENCE = (
    ROOT /
    "datasets/harmony_parts_v1/pre_roof_qc/"
    "view_left_structure_reference_v2_current_pose/rgb.jpg"
)

STRUCTURE_CONFIG = (
    ROOT /
    "datasets/harmony_parts_v1/pre_roof_qc/"
    "view_left_structure_thresholds_v2_current_pose.json"
)
...
# =========================================================
# LEFT V4 STRUCTURE QC
#
# Main judgement:
#   structural template correlation
#
# Absolute RGB brightness and legacy depth difference
# are diagnostic only.
#
# Saved reference came directly from /raw, therefore it
# is rotated 180 degrees here to match LEFT canonical view.
# =========================================================

structure_cfg = json.loads(
    STRUCTURE_CONFIG.read_text(
        encoding="utf-8"
    )
)

_structure_ref_cache = None


def get_structure_reference():

    global _structure_ref_cache

    if _structure_ref_cache is not None:
        return _structure_ref_cache

    img = cv2.imread(
        str(STRUCTURE_REFERENCE),
        cv2.IMREAD_COLOR
    )

    if img is None:
        raise RuntimeError(
            "LEFT structure reference load failed"
        )

    if img.shape[:2] != (720, 1280):
        raise RuntimeError(
            f"LEFT structure reference bad shape: {img.shape}"
        )

    # Robot Control /raw is upside-down.
    # LEFT runtime canonical image is ROTATE_180.
    img = cv2.rotate(
        img,
        cv2.ROTATE_180
    )

    _structure_ref_cache = img
...
def structure_match(
    ...
    margin = int(
        structure_cfg["search_margin_px"]
    )
...
def evaluate(
    ...
    for item in rois:
        ...
        roi_fail = structure_fail

        # ---------------------------------------------
        # Existing LEFT CORNER_LEFT lift detector.
        # Threshold policy stays unchanged.
        # Reference is rebased to current LEFT pose.
        # ---------------------------------------------
        ...
        results[name] = {
            ...
            "rgb": {
                "structure_best_match": {
                    ...
                    "mode":
                        "NORMALIZED_GRAY_CORRELATION",
                },

                "legacy_golden_diagnostic":
                    old_rgb_detail,
            },
...
<head>
<meta charset="utf-8">
<title>Harmony PRE-ROOF LEFT QC</title>
</head>
