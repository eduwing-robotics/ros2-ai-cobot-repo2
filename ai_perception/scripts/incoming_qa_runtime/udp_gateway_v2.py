...
class IncomingQaUdpGatewayV2(Node):
    ...
    def poll_udp(self):
        while True:
            ...
            # HOUSE_B new-request race guard only.
            # Deliver to the ROS2 runtime before the server receives ACK,
            # so V7 can queue the pending request before operator SPACE.
            if (
                not duplicate
                and request.inspection_mode.value == "HOUSE_B"
            ):
                msg = String()
                ...
                print(
                    "HOUSE_B_REQUEST_PUBLISHED_BEFORE_ACK "
            ...
            if duplicate:
                ...
                # HOUSE_B recovery only.
                # Re-publish an accepted duplicate request when no
                # final result exists, so a restarted V7 runtime can
                # restore its in-memory pending request.
                if (
                    request.inspection_mode.value == "HOUSE_B"
                    and existing is not None
                    and existing["runtime_status"] == "ACCEPTED"
                ):
                    msg = String()
                    ...
                    print(
                        "HOUSE_B_DUPLICATE_REPUBLISHED_TO_RUNTIME "
                        f"id={request.inspection_request_id} "
                        f"cycle={request.inspection_cycle}",
                        flush=True,
                    )
