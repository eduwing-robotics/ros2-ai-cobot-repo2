...
class ZK:
    def __init__(self, port=None):
        # ★ 2026-08-10 C1: 기본값이 /dev/ttyUSB0 이었다. ttyUSB 번호는 어댑터
        #   삽입 순서로 바뀌어서, 어느 날은 그게 2호기다(8/8 실제 사고).
        #   포트를 안 준 호출은 프로파일 기본 개체(zkbot1)의 by-path 로 보낸다.
        if port is None:
            import zk_profiles
            port = zk_profiles.get().port
