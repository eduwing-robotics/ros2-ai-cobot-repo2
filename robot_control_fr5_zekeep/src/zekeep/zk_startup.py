...
이 스크립트가 그 둘을 자동으로 처리한다.
    ...
    ~/zkbot_venv/bin/python ~/zk_startup.py           # 점검 + 복구 + 원점복귀
    ~/zkbot_venv/bin/python ~/zk_startup.py --check   # 점검만 (움직이지 않음)
"""
import struct
import subprocess
import sys
...
# ★ 2026-08-10 C1: 이 스크립트가 잡은 개체를 한 번만 정하고 전 구간에서 쓴다.
PROF = ZPROF.resolve_from_argv()


def run(script, *args):
    print(f"\n  $ {script} {' '.join(args)}", flush=True)
    # ★ 서브프로세스에 개체를 반드시 인계한다 — 빠뜨리면 escape/home 이
    #   기본 로봇으로 가버린다(ROS2 노드에서 실제로 터진 버그와 동일).
    r = subprocess.run([PY, f"/home/ar/{script}"] + list(args)
                       + ["--robot", PROF.name, "--port", PROF.port],
...
def main():
    ...
    print("\n" + "=" * 60)
    if len(homed) == 3:
        print("✅ 준비 완료 — 세 축 원점 확보")
        print("   다음: ~/zkbot_venv/bin/python ~/zk_pose.py goto pick1")
    else:
        print(f"🔴 원점 미확보: {[j for j in JOINT_D if j not in homed]}")
        print("   → 해당 축이 리밋을 못 찾는 경우:")
        print("     ~/zkbot_venv/bin/python ~/zk_seek_limit.py <축> --cap 80")
        print("     그 뒤 ~/zkbot_venv/bin/python ~/zk_home_run.py --force")
    print("=" * 60)
    zk.close()
